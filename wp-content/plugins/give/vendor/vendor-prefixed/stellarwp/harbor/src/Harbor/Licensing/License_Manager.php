...
use Give\Vendors\LiquidWeb\Harbor\Traits\With_Error_Throttle;
...
class License_Manager {
	...
	public function validate_and_store( string $key, string $domain, bool $network = false ) {
		...
		if ( ! License_Key::is_valid_format( $key ) ) {
			...
			return new WP_Error(
				...
				__( 'The license key format is invalid.', 'give' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( $this->repository->get_recent_failure_count( self::VALIDATION_FAILURE_WINDOW ) >= self::VALIDATION_FAILURE_THRESHOLD ) {
			...
			return new WP_Error(
				...
				__( 'Too many failed license activation attempts. Please wait a moment and try again.', 'give' ),
		...
		if ( ! $this->repository->store_key( $key, $network ) ) {
			...
			return new WP_Error(
				...
				__( 'The license key could not be stored.', 'give' ),
				[ 'status' => 500 ]
			);
		}
	...
	public function get_products( string $domain ) {
		...
		if ( $key === null ) {
			return new WP_Error(
				Error_Code::INVALID_KEY,
				__( 'No license key is stored.', 'give' ),
				[ 'status' => 422 ]
			);
		}
	...
	private function call_products_api( string $key, string $domain ) {
		try {
			...
			return new WP_Error( Error_Code::INVALID_RESPONSE, __( 'An unexpected error occurred.', 'give' ), [ 'status' => 500 ] );
		}
	}
